...
VerificationResult _verified() => VerificationResult.verified(
  Tier1Payload.decode(
    Uint8List.fromList([
      1,
      8,
...
ReaderController _controller(VerificationResult result) => ReaderController(
  ...
  tier2Repository: Tier2Repository(
    const ApiConfig(baseUrl: '', clinicianToken: ''),
  ),
);

void main() {
  testWidgets(
    'access portal exposes citizen, doctor, and emergency-reader surfaces',
    (tester) async {
      ...
      await tester.ensureVisible(find.text('Citizen'));
  ...
  testWidgets('valid NFC verification reveals the Tier 1 emergency profile', (
    tester,
  ) async {
    await tester.pumpWidget(
      MediTagApp(
        controller: _controller(_verified()),
        initialSurface: AppSurface.reader,
      ),
    );
  ...
  testWidgets('invalid verification has an urgent, distinct warning', (
    tester,
  ) async {
    await tester.pumpWidget(
      MediTagApp(
    ...
    expect(
      find.text('Signature invalid — do not trust this tag'),
      findsOneWidget,
    );
  });

  testWidgets(
    'issue form requires input before displaying its destructive-write confirmation',
    (tester) async {
      await tester.pumpWidget(
        MediTagApp(
          controller: _controller(_verified()),
      ...
      await tester.pump();
      await tester.ensureVisible(find.text('Review tag write'));
      await tester.tap(find.text('Review tag write'));
      await tester.pumpAndSettle();
      expect(find.text('Confirm tag write'), findsOneWidget);
      expect(
        find.textContaining('will not write to NFC hardware'),
        findsOneWidget,
      );
    },
  );
