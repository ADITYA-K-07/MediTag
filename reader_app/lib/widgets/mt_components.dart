...
enum MTCardVariant { plain, flat, emphasised, dark, orchid }

enum MTButtonVariant { primary, secondary, accent, text }
...
class MTFocusRing extends StatefulWidget {
  const MTFocusRing({
    super.key,
    required this.child,
    this.borderRadius = MTTokens.shapeMd,
  });
  final Widget child;
  ...
  Widget build(BuildContext context) => FocusableActionDetector(
    ...
    child: DecoratedBox(
      decoration: BoxDecoration(
        ...
        boxShadow: _focused ? MTTokens.focusGlow : const [],
      ),
      child: widget.child,
    ),
  );
...
class MTCard extends StatelessWidget {
  const MTCard({
    super.key,
    required this.child,
    this.variant = MTCardVariant.plain,
  ...
  Widget build(BuildContext context) {
    final shadows = switch (variant) {
      MTCardVariant.flat || MTCardVariant.orchid => const <BoxShadow>[],
      MTCardVariant.emphasised || MTCardVariant.dark => MTTokens.level2,
      MTCardVariant.plain => MTTokens.level1,
    };
    final background = switch (variant) {
      MTCardVariant.flat => MTTokens.cardMuted,
      MTCardVariant.dark => MTTokens.inkBrand,
      MTCardVariant.orchid => MTTokens.orchidWash,
      _ => MTTokens.card,
    };
    final content = DecoratedBox(
      ...
      child: DefaultTextStyle.merge(
        style: TextStyle(
          color: variant == MTCardVariant.dark ? MTTokens.card : MTTokens.ink,
        ),
...
class MTBrand extends StatelessWidget {
  const MTBrand({super.key, this.compact = false, this.onTap});
  final bool compact;
  final VoidCallback? onTap;
  ...
  Widget build(BuildContext context) {
    final mark = DecoratedBox(
      decoration: BoxDecoration(
        color: MTTokens.inkBrand,
        borderRadius: BorderRadius.circular(compact ? 11 : 13),
        boxShadow: MTTokens.level2,
      ),
      child: SizedBox(
        width: compact ? 34 : 40,
...
class MTOverline extends StatelessWidget {
  const MTOverline({
    super.key,
    required this.label,
    this.tone = MTBadgeTone.neutral,
    this.dot = true,
    this.onDark = false,
  });
  final String label;
    ...
    children: [
      if (dot) ...[
        DecoratedBox(
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: _toneColor(tone).withValues(alpha: .18),
                spreadRadius: 4,
              ),
            ],
          ),
      ...
      Flexible(
        child: Text(
          ...
          style: MTTokens.labelCaps.copyWith(
            color: onDark ? MTTokens.onDarkMuted : MTTokens.inkMuted,
            letterSpacing: 1.05,
            fontWeight: FontWeight.w700,
          ),
        ),
...
class MTAvatar extends StatelessWidget {
  const MTAvatar({super.key, required this.initials, this.large = false});
  final String initials;
  final bool large;
  @override
  Widget build(BuildContext context) => DecoratedBox(
    decoration: BoxDecoration(
      color: MTTokens.containerHigh,
...
class MTIconTile extends StatelessWidget {
  const MTIconTile({
    super.key,
    required this.icon,
    this.tint = MTTint.blue,
    this.size = 48,
  });
  final IconData icon;
  final MTTint tint;
  final double size;
  Color get _wash => switch (tint) {
    MTTint.navy => MTTokens.navyWash,
    MTTint.orchid => MTTokens.orchidWash,
    MTTint.amber => MTTokens.amberWash,
    MTTint.blue => MTTokens.blueWash,
    MTTint.clay => MTTokens.clayWash,
    MTTint.mint => MTTokens.mintWash,
  };
  Color get _ink => switch (tint) {
    MTTint.orchid => MTTokens.orchid,
    MTTint.amber => MTTokens.amber,
    MTTint.clay => MTTokens.clay,
    MTTint.mint => MTTokens.mint,
    _ => MTTokens.periwinkle,
  };
  @override
  Widget build(BuildContext context) => DecoratedBox(
    decoration: BoxDecoration(
    ...
    child: SizedBox(
      ...
      height: size,
      child: Center(
        child: PhosphorIcon(icon, color: _ink, size: size * .46),
      ),
    ),
  );
}
        ...
        child: Column(
          ...
          children: [
            ...
            if (subtitle case final subtitle?) ...[
              ...
              Text(subtitle, style: const TextStyle(color: MTTokens.inkMuted)),
            ],
          ],
        ),
      ),
...
class MTSectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
    children: [
      Expanded(child: Text(title, style: MTTokens.headlineMd)),
      ?action,
    ],
  );
}
...
class MTPageHeading extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final copy = Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(title, style: MTTokens.headlineXl),
        if (subtitle != null) ...[
...
class MTButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final accent = variant == MTButtonVariant.accent;
    ...
    final button = TextButton.icon(
      ...
      style: TextButton.styleFrom(
        ...
        backgroundColor: primary
            ? MTTokens.inkBrand
            : (accent ? MTTokens.accentBlue : (text ? null : MTTokens.card)),
  ...
  Widget build(BuildContext context) => MTFocusRing(
    ...
    child: IconButton(
      ...
      style: IconButton.styleFrom(
        ...
        shape: const RoundedRectangleBorder(borderRadius: MTTokens.shapeSm),
      ),
    ),
  );
}
      ...
      destinations: [
        for (final item in items)
          NavigationDestination(
            icon: PhosphorIcon(item.icon, color: MTTokens.inkMuted),
            selectedIcon: PhosphorIcon(item.icon, color: MTTokens.periwinkle),
            label: item.label,
          ),
      ],
    ),
    ...
    child: DecoratedBox(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            if (icon != null) ...[
              PhosphorIcon(icon!, size: 16, color: _toneColor(tone)),
...
class MTVerificationBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final title = valid
        ? 'Verified offline'
        : 'Signature invalid — do not trust this tag';
    return Semantics(
      liveRegion: !valid,
      label: valid
          ? 'Verified offline'
          : 'Urgent: signature invalid. Do not trust this tag.',
      child: DecoratedBox(
        decoration: BoxDecoration(
          color: _toneWash(tone),
          borderRadius: MTTokens.shapeLg,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      detail ??
                          (valid
                              ? 'This emergency information matches its issuer signature.'
                              : "This tag's data doesn't match its signature — it may have been altered."),
                      style: const TextStyle(color: MTTokens.ink, height: 1.35),
                    ),
                  ],
                ),
              ),
...
class MTTag extends StatelessWidget {
  ...
  Widget build(BuildContext context) => DecoratedBox(
    ...
    child: Padding(
      ...
      child: Text(
        label,
        style: MTTokens.monoData.copyWith(
          color: MTTokens.periwinkle,
          fontWeight: FontWeight.w700,
        ),
      ),
  ...
  Widget build(BuildContext context) => MTCard(
    ...
    child: Row(
      children: [
        DecoratedBox(
          decoration: const BoxDecoration(
            color: MTTokens.darkPanel,
            borderRadius: MTTokens.shapeMd,
          ),
          child: Padding(
            padding: const EdgeInsets.all(13),
            child: PhosphorIcon(icon, color: MTTokens.accentBlue, size: 24),
          ),
        ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(
                label.toUpperCase(),
                style: MTTokens.labelCaps.copyWith(color: MTTokens.onDarkMuted),
              ),
              const SizedBox(height: 2),
              Text(
                value,
                style: MTTokens.statNumber.copyWith(color: MTTokens.card),
              ),
            ],
          ),
        ),
...
class MTTintTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Semantics(
    ...
    child: DecoratedBox(
      decoration: BoxDecoration(
        color: critical ? _toneWash(MTBadgeTone.critical) : _wash,
        borderRadius: MTTokens.shapeMd,
      ),
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            PhosphorIcon(
              icon,
              size: 19,
              color: critical ? MTTokens.criticalRed : MTTokens.inkBrand,
            ),
  ...
  Widget build(BuildContext context) => MTCard(
    ...
    child: Column(
      ...
      children: [
        ...
        Text(
          reason,
          style: const TextStyle(color: MTTokens.inkMuted, height: 1.4),
        ),
        if (onRetry != null) ...[
        ...
        children: [
          DecoratedBox(
            ...
            child: Padding(
              ...
              child: PhosphorIcon(icon, color: MTTokens.periwinkle, size: 30),
            ),
          ),
          const SizedBox(height: 18),
          Text(title, style: MTTokens.headlineLg, textAlign: TextAlign.center),
          const SizedBox(height: 8),
          Text(
            message,
...
class MTEmptyNote extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MTCard(
    ...
    child: Row(
      children: [
        PhosphorIcon(icon, color: MTTokens.inkMuted),
        const SizedBox(width: 10),
        Expanded(
          child: Text(
            message,
            style: const TextStyle(color: MTTokens.inkMuted),
          ),
        ),
      ],
    ),
  ...
  Widget build(BuildContext context) => Row(
    children: [
      ...
      Switch(
        value: value,
        onChanged: onChanged,
        activeTrackColor: MTTokens.periwinkle,
      ),
    ],
  );
}
      ...
      TextFormField(
        ...
        decoration: InputDecoration(
          ...
          focusedBorder: const OutlineInputBorder(
            borderRadius: MTTokens.shapeMd,
            borderSide: BorderSide(color: MTTokens.periwinkle, width: 2),
          ),
        ),
      ),
    ],
...
class MTModal {
  static Future<T?> show<T>({
    ...
    builder: (_) => Dialog(
      backgroundColor: MTTokens.card,
      elevation: 0,
      shape: const RoundedRectangleBorder(borderRadius: MTTokens.shapeXl),
      child: DecoratedBox(
        decoration: const BoxDecoration(
          borderRadius: MTTokens.shapeXl,
          boxShadow: MTTokens.level3,
        ),
...
class MTEnter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return TweenAnimationBuilder<Offset>(
      ...
      duration: reduced
          ? Duration.zero
          : MTTokens.medium + Duration(milliseconds: (index.clamp(0, 5)) * 60),
