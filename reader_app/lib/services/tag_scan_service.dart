...
class NfcTagScanService implements TagScanService {
  NfcTagScanService({required this.publicKeyBase64, NfcReader? reader}) : _reader = reader ?? NfcReader();
  ...
  Future<VerificationResult> scanAndVerify() async {
    if (publicKeyBase64.isEmpty) {
      throw StateError('This build has no issuer public key. Provision MEDITAG_PUBLIC_KEY before scanning real tags.');
    }
    final key = TagVerifier.publicKeyFromUncompressed(Uint8List.fromList(base64Decode(publicKeyBase64)));
